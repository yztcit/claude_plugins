...
echo "[INFO] Step 1/2: 刷新 tal-tools marketplace..."
if claude plugin marketplace update tal-tools; then
...
echo "[INFO] Step 2/2: 更新 dev@tal-tools 插件..."
claude plugin update dev@tal-tools -s project || true
claude plugin update dev@tal-tools -s user || true
...
echo "验证: claude plugin list --json | grep dev@tal-tools"
