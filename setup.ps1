...
if (Test-Path $ruleFile) {
    ...
    $searchRuleUrl = "https://raw.githubusercontent.com/yztcit/claude_plugins/main/plugins/dev/skills/graphify-search/search-rule.md"
    try {
        Invoke-WebRequest -Uri $searchRuleUrl -OutFile $ruleFile -UseBasicParsing
        Write-Ok "搜索规则已下载: $ruleFile"
    } catch {
        Write-Fail "下载搜索规则失败: $_"
    }
}
