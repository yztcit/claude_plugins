...
if [ -f "$RULE_FILE" ]; then
  ...
  SEARCH_RULE_URL="https://raw.githubusercontent.com/yztcit/claude_plugins/main/plugins/dev/skills/graphify-search/search-rule.md"
  if curl -fsSL "$SEARCH_RULE_URL" -o "$RULE_FILE"; then
    ok "搜索规则已下载: $RULE_FILE"
  else
    fail "下载搜索规则失败（$SEARCH_RULE_URL）"
  fi
fi
